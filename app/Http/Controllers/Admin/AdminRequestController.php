    ...
    function check(Request $request){
        {
            ...
            if (count($dates) == 0) {
                return redirect("/request")->with('error_message', '日付が指定されていません');
            }
            $type = RequestTypes::find($request->type);
            if ($type == null) {
                return redirect("/request")->with('error_message', '種別が指定されていません');
            }
            $holidays = 0;
            $time = NULL;
            if (isset($request->time) && $type->type == 1) {
                ...
                if ($time == null || $time == "00:00") {
                    return redirect("/request")->with('error_message', '労働時間が指定されていません');
                }
            }
            if ($request->type == 2) {
            ...
            if ($holidays > $user->paid_holiday) {
                return redirect("/request")->with('error_message', '有給消費が残日数を超えています');
            }
            ...
            return redirect("/admin/request")->with('flash_message', '申請を行いました');
